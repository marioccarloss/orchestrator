...
3. When deterministic evidence leaves material ambiguity, call \`mr_decision_evaluate\` once with a bounded profile and compact evidence. Jev is shadow-only and never overrides the FSM, gates, explicit denies, or human confirmation.
4. Advance through the deterministic phases (SDD + RPI: the AI only produces/consumes compact typed JSON; user-facing markdown is rendered by script via the mr_sdd_* tools):
